package by.it.group151003.onuchina.lesson08;
...
public class B_Knapsack {
    ...
    int getMaxWeight(InputStream stream ) {
        ...
        int[] arr_fill_pos = new int[w + 1];

        arr_fill_pos[0] = 1; //With 0 knapsack capacity it's always possible to fill it in completely
        int i;
        for (i = 0; i < gold.length; i++) //All gold bars
            for(int j = w; j >= gold[i]; j--) //All positions from max weight to weight of current gold bar
                if (arr_fill_pos[j - gold[i]] == 1) //If we have space for new gold bar then set it in array
                    arr_fill_pos[j] = 1;
