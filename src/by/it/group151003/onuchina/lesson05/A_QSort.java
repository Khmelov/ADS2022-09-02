...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < m; i++){
            result[i] = count_registrators(segments, points[i]);
        }
        return result;
    }
    ...
    private int count_registrators(Segment[] segments, int point) {
        int counter = 0;
        for (Segment segment : segments) {
            if (point >= segment.start && point <= segment.stop) counter++;
        }
        return counter;
    }
