package by.it.group151003.baranouski.lesson02;
...
public class Lesson2Test {
    ...
    */
    ...
    public void A_VideoRegistrator() {
        ...
        double[] events2 = new double[]{0};
        starts = instance.calcStartTimes(events2, 10);
        ok = starts.toString().equals("[0.0]");
        assertTrue("A_VideoRegistrator failed at 0", ok);
    }
    ...
    public void B_Sheduler() {
        ...
        B_Sheduler.Event[] events2 = {new B_Sheduler.Event(0, 3), new B_Sheduler.Event(3, 12)};
        starts = instance.calcStartTimes(events2, 0, 11);
        ok=starts.toString().equals("[(0:3)]");
        assertTrue("B_Sheduler failed at 1 event", ok);
    }

    @Test
    public void C_GreedyKnapsack() throws Exception {
        String root=System.getProperty("user.dir")+"/src/";
        File f=new File(root+"by/it/group151003/baranouski/lesson02/greedyKnapsack.txt");
        double costFinal=new C_GreedyKnapsack().calc(f);
        boolean ok=C_GreedyKnapsack.isEqualDoubles(costFinal, 200.0, 0.01, 0.0001);
        assertTrue("C_GreedyKnapsack failed 1", ok);
        f = new File(root+"by/it/group151003/baranouski/lesson02/greedyKnapsack2.txt");
        costFinal = new C_GreedyKnapsack().calc(f);
        ok=C_GreedyKnapsack.isEqualDoubles(costFinal, 286.666667, 0.01, 0.0001);
        assertTrue("C_GreedyKnapsack failed 2", ok);
        f = new File(root+"by/it/group151003/baranouski/lesson02/greedyKnapsack3.txt");
        costFinal = new C_GreedyKnapsack().calc(f);
        ok=C_GreedyKnapsack.isEqualDoubles(costFinal, 30.0, 0.01, 0.0001);
