...
public class C_EditDist {
    ...
    String getDistanceEdinting(String one, String two) {
        ...
        //System.out.println(EditMatrix[one.length()][two.length()]);

        String revresult = "", result = "";
        for(int i = one.length(), j = two.length(); i!=0 || j!=0; i--, j-- ){
            if(EditMatrix[i][j] == EditMatrix[i][j-1]+1){
