...
public class TaskB {
    ...
    public static void main(String[] args) {
        ...
        int Vert = (int)((in.next().toUpperCase().charAt(0)) - START_VERTEX);

        taskB.shortestPaths(Vert);

        for (int i = 0; i < AMOUNT_OF_VERTICES; i++) {
            char nextVertex = (char) (START_VERTEX + i);
            if (nextVertex == 'J')
                nextVertex = 'S';
            System.out.print("The shortest path from " + (char)(START_VERTEX + Vert) + " to " + nextVertex + " = " + (taskB.dist[i] > taskB.MAX - 9999 ? "(no path)" : taskB.dist[i]));
            System.out.print("\n");
        }
    }
}
