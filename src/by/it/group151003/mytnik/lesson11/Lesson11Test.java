...
public class Lesson11Test {
    ...
    public void taskA() {
        ...
        String expected = """
                A - B (tree) B - A (back) B - C (tree) C - B (back) C - F (tree) F - C (back) F - E (tree) E - A (back) E - B (back) E - F (back) F - I (tree) I - F (back)\s
                D - G (tree) G - D (back) G - H (tree) H - D (back) H - G (back)\s
                """;
        TaskA.dfs(graph, 0);

        System.out.println(TaskA.result.toString());

        assertEquals("Failed", expected, TaskA.result.toString());
    }
