package by.it.group151003.denisenko.lesson12;
...
public class TaskB {
    ...
    public int[] m_dist = new int[10];
    public int[] m_path = new int[10];
    ...
    public void bellmanFord(int s) {
        ...
        Arrays.fill(m_path, -1);
        m_dist[s] = 0;

        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 10; j++) {
                for (int k = 0; k < 10; k++) {
                    if (m_dist[k] > m_dist[j] + matrix[j][k]) {
                        m_dist[k] = m_dist[j] + matrix[j][k];
                        m_path[k] = j;
                    }
                }
            }
