package by.it.group151001.kononova.lesson03;
...
public class A_Huffman {
    ...
    public String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            char simbol = s.toCharArray()[i];
            if (count.containsKey(simbol)) {
                count.put(simbol, count.get(simbol)+1);
            } else {
                count.put(simbol, 1);
            }
        ...
        for (Map.Entry<Character, Integer> start : count.entrySet()) {
            LeafNode node = new LeafNode(start.getValue(), start.getKey());
