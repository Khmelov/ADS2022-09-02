package by.it.group151001.shcherba.lesson05;
...
public class C_QSortOptimized {
    ...
    public static void swap(Segment[] a, int i, int j) {
        if (i != j) {
            Segment temp = a[i];
            a[i] = a[j];
            a[j] = temp;
    ...
    public static void qsort(Segment[] arr, int left, int right){
        while (left < right){
            ...
            if (piv[0] - left < right - piv[1]) {
                qsort(arr, left, piv[0] - 1);
                left = piv[1] + 1;
            } else {
                qsort(arr, piv[1] + 1, right);
                right = piv[0] - 1;
            }
        }
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/group151001/shcherba/lesson05/dataC.txt");
