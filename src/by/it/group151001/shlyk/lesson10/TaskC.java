...
import by.it.group151001.shlyk.lesson04.C_GetInversions;
...
public class TaskC<E>  implements NavigableSet<E> {
    ...
    interface DummyNode<E> {
        ...
        DummyNode<E> getSon(SonType type);
        int compareTo(E value);
    }
    ...
    public TaskC(Comparator<E> comparator) {
    ...
    interface TreeCondAction {
        int cond(DummyNode node);
    }
    public TreeNode findNodeCond(TreeNode node, TreeCondAction handler){
        if(handler.cond(node) == 0)
            return node;
        if(handler.cond(node) < 0)
            return findNodeCond(node.lSon, handler);
        else
            return findNodeCond(node.rSon, handler);
    }
    ...
    DummyNode lastTouched = null;

    public boolean lowerCond(DummyNode node){
        if(lastTouched == null)
            return false;
        return node.compareTo(lastTouched.getValue()) > 0;
    }
    public boolean higherCond(DummyNode node){
        if(lastTouched == null)
            return false;
        return node.compareTo(lastTouched.getValue()) < 0;
    }
    interface MemoryAction {
        void save(DummyNode node, boolean updValue);
    }
    MemoryAction memoAct = (node, updValue) -> {
        if(lastTouched == null){
            lastTouched = node;
        }
        else {
    ...
    public E lower(E e) {
        if(root == null)
            return null;
        lastTouched = null;
        TreeCondAction lowerAct = (node) -> {
            if(node.getSon(SonType.Random) == null) // it's leaf
                return 0;

            int cmpResult = node.compareTo(e);

            if(cmpResult >= 0 && node.getSon(SonType.Left) != null)
                return -1;
            if(cmpResult < 0){
                memoAct.save(node, lowerCond(node));
                if(node.getSon(SonType.Right) != null)
                    return 1;
            }
            return 0;
        };
        E result;
        TreeNode probe = findNodeCond(root, lowerAct);
        if(lastTouched != null){
            if(probe.compareTo(e) == 0)
                result = (E)lastTouched.getValue();
            else
                result = lastTouched.compareTo(probe.getValue()) > 0 ? (E) lastTouched.getValue() : probe.getValue();
        }
       else
           result = probe.getValue();

       if( ((Comparable<E>) e).compareTo(result) <= 0)
           result = null;
        return result;
    }
    ...
    public E floor(E e) {
        ...
        TreeNode probe = findNodeCond(root, floorAct);
        E result;
        if(lastTouched != null)
            result = lastTouched.compareTo(probe.getValue()) > 0 ? (E) lastTouched.getValue() : probe.getValue();
        else
    ...
    public E higher(E e) {
        ...
        lastTouched = null;
        TreeCondAction higherAct = (node) -> {
            if(node.getSon(SonType.Random) == null) // it's leaf
                return 0;

            int cmpResult = node.compareTo(e);
            if(cmpResult > 0){
                memoAct.save(node, higherCond(node));
                if(node.getSon(SonType.Left) != null)
                    return -1;
            }
            if(cmpResult <= 0 && node.getSon(SonType.Right) != null){
                return 1;
            }
            return 0;
        };
        TreeNode probe = findNodeCond(root, higherAct);
        E result;
        if(lastTouched != null) {
            if(probe.compareTo(e) == 0)
                result = (E)lastTouched.getValue();
