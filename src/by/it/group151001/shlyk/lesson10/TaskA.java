...
public class TaskA<E>  implements NavigableSet<E> {
    ...
    private class TreeNode{
        ...
        public void updateFather(){
            if(father != null && (father.height != this.height + 1)){
                father.height = this.height + 1;
                father.updateFather();
            }
        }
    ...
    public TaskA(Comparator<E> comparator) {
    ...
    private boolean insertChild(TreeNode currNode, E value){
        ...
        if(currNode.compareTo(value) < 0){
            wasAdded = (currNode.rSon == null);
            child = (wasAdded) ? new TreeNode(value, currNode) : currNode.rSon;
            order = SonType.Right;
        }
        else {
            wasAdded = (currNode.lSon == null);
            child = (currNode.lSon == null) ? new TreeNode(value, currNode) : currNode.lSon;
            order = SonType.Left;
        }
        if(wasAdded) {
            lastChild = child;
            ...
            wasAdded = insertChild(child, value);
        }
        return  wasAdded;
    }
...
//        }
    ...
    public boolean remove(Object o) {
        ...
        if(result != root){
            ...
            if(root.lSon == null || root.rSon == null){
                TreeNode dummyHead = new TreeNode(root.value);
    ...
    private void concateInOrder(TreeNode root){
        if(root.lSon != null)
            concateInOrder(root.lSon);
        strOutput.append(root.value.toString()).append(", ");
        if(root.rSon != null)
            concateInOrder(root.rSon);
    }
    @Override
    ...
    private class TreeIterator implements Iterator<E> {
        ...
        public E next() {
            ...
            if(currNode.rSon == null){
                ...
                currNode = getLeaf(currNode.rSon, SonType.Left);
            }
            return result;
        }
        TreeIterator(){
            if(root == null) {
                ...
                currNode = getLeaf(root, SonType.Left);
            }
        }
    }
    @Override
