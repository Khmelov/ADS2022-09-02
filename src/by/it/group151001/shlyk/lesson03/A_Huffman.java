...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for(Map.Entry word : count.entrySet()){
            priorityQueue.add(
                    new LeafNode((int) word.getValue(), (char) word.getKey())
            );
        }
        InternalNode sample = null;
        while(priorityQueue.size() != 1){
            sample = new InternalNode(
                    priorityQueue.remove(), priorityQueue.remove()
            );
            priorityQueue.add(sample);
        }
        ...
        String appendix = "";
        sample.fillCodes(appendix);
        for(int i = 0; i < s.length(); i++){
            sb.append(
                    codes.get(s.charAt(i))
            );
        }
        return sb.toString();
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "by/it/group151001/shlyk/lesson03/dataHuffman.txt");
