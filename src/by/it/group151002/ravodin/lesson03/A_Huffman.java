...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int value, i = 0; i < s.length(); i++) {
        ...
        while (priorityQueue.size() != 1){
            tmp1 = priorityQueue.peek();
            priorityQueue.poll();
            tmp2 = priorityQueue.peek();
            priorityQueue.poll();
            if (tmp1.frequence <= tmp2.frequence)
                priorityQueue.add(new InternalNode(tmp1, tmp2));
            else
                priorityQueue.add(new InternalNode(tmp2, tmp1));
        }
