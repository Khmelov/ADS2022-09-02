...
public class A_Huffman {
    ...
    Map<Character, Integer> constructFrequencyMap(String s){
        Map<Character, Integer> map = new HashMap<>();
        int temp;
        for(int i=0;i<s.length();i++){
            temp = map.getOrDefault(s.charAt(i), 0) + 1;
            map.put(s.charAt(i), temp);
        }
        return map;
    }

    PriorityQueue<Node> constructPriorityQueue(Map<Character, Integer> currMap){
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for(Map.Entry<Character,Integer> entrySet : currMap.entrySet()){
            priorityQueue.add(new LeafNode(entrySet.getValue(), entrySet.getKey()));
        }
        return priorityQueue;
    }

    Node createHuffmanTree(PriorityQueue<Node> priorityQueue){
        Node firstNode, secondNode;
        while (priorityQueue.size() != 1){
            firstNode = priorityQueue.poll(); //изъятие (с удалением)
            secondNode = priorityQueue.poll();
            InternalNode internalNode = new InternalNode(firstNode, secondNode);
            priorityQueue.add(internalNode); //добавление в конец очереди
        }
        return priorityQueue.peek(); //получение корня дерева(без удаления)
    }
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        Map<Character, Integer> frequencyMap = constructFrequencyMap(s);

        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = constructPriorityQueue(frequencyMap);
