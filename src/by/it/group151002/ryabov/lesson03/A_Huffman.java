    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++){
            if (!count.containsKey(s.charAt(i))){
                ...
                v = count.get(s.charAt(i));
                count.replace(s.charAt(i), v, v + 1);
            }
        }
        ...
        Character[] arr = {};
        Set<Character> st = new HashSet<>(count.keySet());
        arr = st.toArray(new Character[count.size()]);
        for (int i = 0; i < count.size(); i++){
            priorityQueue.add(new LeafNode(count.get(arr[i]), arr[i]));
        }
