...
public class A_Huffman {
    ...
    void getHuffmanCodeTree (PriorityQueue<Node> priorityQueue) {
        if(priorityQueue != null && priorityQueue.size() > 1) {
            Node firstNode = priorityQueue.poll();
            Node secondNode = priorityQueue.poll();
            InternalNode insertNode = new InternalNode(firstNode, secondNode);
            priorityQueue.add(insertNode);
            getHuffmanCodeTree(priorityQueue);
        }
    }
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < inputString.length(); i++) {
            count.putIfAbsent(inputString.charAt(i), 0);
            count.put(inputString.charAt(i), count.get(inputString.charAt(i) ) + 1);
        }
        ...
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            LeafNode insertNode = new LeafNode(entry.getValue(), entry.getKey());
            priorityQueue.add(insertNode);
        }
        ...
        Node huffmanCodeTreeRoot = priorityQueue.poll();
        huffmanCodeTreeRoot.fillCodes("");
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < inputString.length(); i++)
            sb.append(codes.get(inputString.charAt(i) ) );
        return sb.toString();
        //01001100100111
        //01001100100111
    }
