...
public class TaskC {
    ...
    public static void dijkstra(int start) {
        ...
        dist[start] = 0;
        PriorityQueue<TaskC.Node> q = new PriorityQueue<>(Comparator.comparingInt(p -> p.cost));
        q.offer(new TaskC.Node(start, 0));
        while (!q.isEmpty()) {
            TaskC.Node node = q.poll();
            int curr = node.dest;
            if (visited[curr])
                continue;
            visited[curr] = true;
            for (int i = 0; i < graph.get(curr).size(); i++) {
                ...
                if (dist[next] > dist[curr] + cost) {
                    ...
                    q.offer(new TaskC.Node(next, dist[next]));
                }
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        dijkstra(v);
        System.out.print("The smallest cycle between " + (char)(u + START_VERTEX) + " and " + (char)(v + START_VERTEX) + ": " + findTheSmallestCycle());
    }
}
