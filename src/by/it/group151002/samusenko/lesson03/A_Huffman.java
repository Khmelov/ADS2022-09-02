...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            count.computeIfPresent(s.charAt(i), (k, v) -> v + 1);
        ...
        boolean needToRemake = true;
        while (needToRemake){
            Arrays.sort(priorityQueue.toArray());
            Node fstLeaf = priorityQueue.poll();
            Node secLeaf = priorityQueue.poll();
            InternalNode parNode = new InternalNode(fstLeaf, secLeaf);
        ...
        String str = "";
        root.fillCodes(str);//это надо куда-то деть
