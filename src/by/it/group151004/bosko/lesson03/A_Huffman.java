package by.it.group151004.bosko.lesson03;
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (Character sym : s.toCharArray()) {
            if (!count.containsKey(sym))
                ...
                count.replace(sym, count.get(sym) + 1);
        }

        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Character sym : count.keySet()){
            priorityQueue.add(new LeafNode(count.get(sym), sym));
        }
        ...
        while(priorityQueue.size() > 1){
            ...
            theParent = new InternalNode(leftChild, rightChild);

            priorityQueue.add(theParent);
        }
