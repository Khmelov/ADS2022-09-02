...
public class C_QSortOptimized {
    ...
    public void swap(Segment[] array, int x, int y)
    {
        Segment temp = array[x];
        array[x] = array[y];
        array[y] = temp;
    }


    public Segment partition(Segment[] array, int left, int right){
        int l = left;
        int current = left;
        int r = right;
        Segment value = array[left];
        while(current <= r) {
            if (array[current].compareTo(value) < 0) {
                swap(array, current, l);
                l++;
                current++;
            } else {
                if (array[current].compareTo(value) == 0)
