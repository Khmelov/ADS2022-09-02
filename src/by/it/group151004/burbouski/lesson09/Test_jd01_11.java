package by.it.group151004.burbouski.lesson09;
...
public class Test_jd01_11 extends HomeWork {
	...
	public void testTaskA__ListA() throws Exception {
		System.out.println("\nA. ƒиагностика об¤зательных к реализации методов:");
	...
	public void testTaskB__ListB() throws Exception {
		System.out.println("\nB. ƒиагностика об¤зательных к реализации методов:");
		...
		System.out.println("\nB. ƒиагностика необ¤зательных к реализации методов:");
		if (e.contains(null) != a.contains(null) || e.contains((long) 0) != a.contains((long) 0) || e.contains((long) 1002) != a.contains((long) 1002)) {
			System.out.println("ƒиагностика: contains не реализован или работает не так как в ArrayList");
		} else {
			System.out.println("      contains(T e) ok: " + a);
		}

		if (e.size() != a.size()) {
			System.out.println("ƒиагностика: size не реализован или работает не так как в ArrayList");
		} else {
			System.out.println("             size() ok: " + a);
		}

		if (e.indexOf((long) 444) != a.indexOf((long) 444)) {
			System.out.println("ƒиагностика: indexOf не реализован или работает не так как в ArrayList");
		} else {
			System.out.println("          indexOf() ok: " + a);
		}

		if (e.remove(Long.valueOf(444)) != a.remove(Long.valueOf(444))) {
			System.out.println("ƒиагностика: remove(Object o) не реализован или работает не так как в ArrayList");
		} else {
	...
	public void testTaskC__SetC() throws Exception {
		System.out.println("\nC. ƒиагностика об¤зательных к реализации методов:");
		...
		assertEquals("ошибка remove(Object o) не работает", e.remove((short) 0), a.remove((short) 0));
		assertSet("ошибка remove(Object o) или toString() не работает", e, a);

		assertEquals("ошибка remove(Object o) не работает", e.remove((short) 8), a.remove((short) 8));
		assertSet("ошибка remove(Object o) или toString() не работает", e, a);

		assertEquals("ошибка remove(Object o) не работает", e.remove((short) 4), a.remove((short) 4));
		assertSet("ошибка remove(Object o) или toString() не работает", e, a);
		...
		assertEquals("ошибка contains не работает", e.contains((short) 101), a.contains((short) 101));
		assertEquals("ошибка contains не работает", e.contains((short) -1), a.contains((short) -1));
