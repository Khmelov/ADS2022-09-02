...
public class Lesson11Test {
    @Test(timeout = 2000)
    public void TaskA() {
        Start graph = new Start(TaskA.number);
        graph.setName(0, 'A');
        graph.setName(1, 'B');
        graph.setName(2, 'C');
        graph.setName(3, 'D');
        ...
        TaskA.dfs(graph, 0);
        assertEquals("Failed", expected, TaskA.tmp);
    }

    @Test(timeout = 2000)
    public void TaskB() {
        Start graph = new Start(TaskB.number);
        graph.setName(0, 'A');
        graph.setName(1, 'B');
        graph.setName(2, 'C');
        graph.setName(3, 'D');
        ...
        TaskB.sort(graph, 0);
        assertEquals("Failed", expected, TaskB.topologic.toString());
    }

    @Test(timeout = 2000)
    public void TaskC() {
        Start graph = new Start(TaskC.number);
        graph.setName(0, 'A');
        graph.setName(1, 'B');
        graph.setName(2, 'C');
        graph.setName(3, 'D');
