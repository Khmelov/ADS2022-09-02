...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for (int i=0;i<segments.length-1;i++){
            result[i] = Count(segments, points[i]);
        }
    ...
    int Count(Segment[] segments, int point){
        int count = 0;
    ...
    int divide(Segment[] segments,int l,int r){
        Segment s = segments[r];
        int i = l-1;
        for (int j=l;j<r;j++){
            if (segments[j].compareTo(s)<=0){
                i++;
        ...
        Segment tmp=segments[i+1];
        segments[i+1] = segments[r];
        segments[r]=tmp;
        return i+1;
    }
