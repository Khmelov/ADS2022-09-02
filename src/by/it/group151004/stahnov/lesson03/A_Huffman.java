package by.it.group151004.stahnov.lesson03;
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for(int i = 0;i < s.length();i++){
            if(count.containsKey(temp[i])){
                count.replace(temp[i],count.get(temp[i])+1);
            }
            else {
                count.put(temp[i],1);
            }
        ...
        Node node0;
        Node node1;
        while(((node0 = priorityQueue.poll()) != null) & ((node1 = priorityQueue.poll()) != null)){
            priorityQueue.add(new InternalNode(node0,node1));
        }
